<div class="max-w-7xl mx-auto px-4 lg:px-8 py-6">
    ...
    <nav class="flex items-center gap-2 text-[12px] font-medium text-gray-500 mb-4">
        ...
        <a href="{{ route('cars.index') }}"
            class="hover:text-gray-900 transition-colors underline decoration-1 underline-offset-2">Cars for Sale</a>
        <span class="text-gray-300">/</span>
    ...
    <div x-data="{ lightbox: false }" @keydown.escape.window="lightbox = false">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-8 h-[300px] md:h-[400px]">
            ...
            <div class="md:col-span-2 relative rounded-xl overflow-hidden bg-gray-100 {{ $car->images->count() ? 'cursor-zoom-in' : '' }}"
                ...
                @if ($car->images->count() > 0)
                    <img src="{{ Storage::url($car->images[$activeImageIndex]->path) }}" class="w-full h-full object-cover"
                        alt="{{ $car->make->name }} {{ $car->carModel->name }}">
                @else
                    <div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-400 text-sm">No Image
                    </div>
                @endif
            ...
            <div class="hidden md:grid grid-cols-2 grid-rows-2 gap-2 h-full">
                @for ($i = 1; $i <= 4; $i++)
                    <button @if ($car->images->count() > $i) wire:click="setActiveImage({{ $i }})" @click="lightbox = true"
                    @endif
                        ...
                        @if ($i == 4 && $car->images->count() > 5)
                            <div
                                class="absolute bottom-2 right-2 bg-white/90 backdrop-blur text-gray-900 text-[12px] font-bold px-3 py-1.5 rounded-full shadow-sm">
        ...
        @if ($car->images->count())
            <div x-show="lightbox" x-cloak x-transition.opacity x-data="{ touchX: 0 }" @click.self="lightbox = false"
                ...
                @if ($car->images->count() > 1)
                    <div class="absolute bottom-4 text-white/70 text-[13px]">{{ $activeImageIndex + 1 }} /
                        {{ $car->images->count() }}
                    </div>
                @endif
            </div>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-8">
        ...
        @if ($this->reservationsCount > 0)
            <span
                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 text-rose-600 text-[13px] font-medium border border-rose-100"
        ...
        <div class="lg:col-span-2 space-y-10">
            ...
            <div>
                <div
                    ...
                    <div class="text-[13px] text-gray-500 mb-5">≈ GH₵{{ number_format($car->price_ghs, 0) }} car price
                    </div>
                ...
                <p class="text-[11px] text-gray-500 mt-3">
                    GHS amounts are estimates based on the current exchange rate and do not include clearing fees, which
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($car->special_features as $feature)
                                <span
                                    class="inline-flex items-center gap-1.5 pl-2 pr-3 py-1.5 rounded-full bg-primary text-white text-[13px] font-semibold shadow-sm">
        ...
        @if ($showOrderModal)
            <x-ui.modal closeAction="closeOrderModal" maxWidth="max-w-md">
                ...
                @if ($this->emailUnverified)
                    <div
                        class="mt-4 rounded-lg bg-red-50 border border-red-200 text-red-800 text-[12px] px-3 py-2 flex items-center justify-between gap-3 flex-wrap">
                ...
                @elseif ($this->kycIncomplete)
                    <div class="mt-4 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-[12px] px-3 py-2">
                        Your KYC isn't complete yet. You can still place this order, but you'll need to finish KYC before your car
                        can be delivered.
                    </div>
                @endif
