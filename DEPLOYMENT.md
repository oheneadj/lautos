...
- [ ] **If your plan won't let you change the document root** (some Hostinger plans pin it to `public_html`), upload the whole app into `public_html` as-is and rely on the root-level `.htaccess` already committed in this repo — it unconditionally rewrites every request into `public/`, so `public/index.php` still runs even though the domain technically points at the project root. This also has the side effect of blocking direct URL access to `.env`, `composer.json`, `database/database.sqlite`, etc., since those requests get rewritten into `public/` too and 404 there instead of being served from the root. Confirm `mod_rewrite` is enabled (it is by default on Hostinger) and that this `.htaccess` actually made it to the server — some FTP clients and zip tools hide/skip dotfiles by default, which is the most common reason this silently goes missing.
