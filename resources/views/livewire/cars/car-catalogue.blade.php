<div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
    ...
    >
        ...
        <div class="relative z-10 ml-auto w-full max-w-xs h-full overflow-y-auto bg-white border border-gray-200 rounded-lg p-5 lg:ml-0 lg:w-auto lg:max-w-none lg:h-auto lg:overflow-visible lg:sticky lg:top-24">
            <div class="flex items-center justify-between mb-1">
                <h2 class="text-base font-bold text-gray-900">{{ $cars->total() }} {{ Str::plural('result', $cars->total()) }}</h2>
                <div class="flex items-center gap-3">
                    @if ($hasActiveFilters)
                        <button wire:click="clearFilters" class="text-xs font-medium text-gray-600 hover:text-gray-900 transition-colors">Clear all</button>
                    @endif
            ...
            @if ($hasActiveFilters)
                <div class="flex flex-wrap gap-2 mb-4 pb-4 border-b border-gray-100">
                    @foreach ($makeFilter as $slug)
                        ...
                        <button wire:click="removeMake('{{ $slug }}')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $makeLabel }}
                    ...
                    @foreach ($modelFilter as $modelName)
                        <button wire:click="removeModel('{{ $modelName }}')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $modelName }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endforeach
                    @foreach ($bodyTypeFilter as $bodyTypeValue)
                        <button wire:click="removeBodyType('{{ $bodyTypeValue }}')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ \App\Enums\CarBodyType::from($bodyTypeValue)->label() }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endforeach
                    @if ($transmissionFilter)
                        <button wire:click="$set('transmissionFilter', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $transmissionFilter }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endif
                    @if ($fuelFilter)
                        <button wire:click="$set('fuelFilter', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $fuelFilter }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endif
                    @if ($countryFilter)
                        <button wire:click="$set('countryFilter', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $countryFilter }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endif
                    @if ($minYear || $maxYear)
                        <button wire:click="$set('minYear', '');$set('maxYear', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            {{ $minYear ?: 'Any' }}&ndash;{{ $maxYear ?: 'Any' }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endif
                    @if ($maxPriceGhs)
                        <button wire:click="$set('maxPriceGhs', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
                            Under GH₵{{ number_format($maxPriceGhs) }}
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    @endif
                    @if ($maxMileage)
                        <button wire:click="$set('maxMileage', '')" class="inline-flex items-center gap-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 transition-colors">
            ...
            <div class="border-t border-gray-100 py-4">
                <button @click="open.make = !open.make" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Make
            ...
            @if (! empty($makeFilter))
                <div class="border-t border-gray-100 py-4">
                    <button @click="open.model = !open.model" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                        Model
            ...
            {{-- Body Type --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.bodyType = !open.bodyType" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Body Type
            ...
            {{-- Price (GHS) --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.price = !open.price" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Car Price (GH₵)
                ...
                <div x-show="open.price" class="mt-3">
                    <input type="number" wire:model.live.debounce.500ms="maxPriceGhs" placeholder="Maximum price" class="w-full bg-gray-100 border-none rounded-lg py-3 px-3 text-center text-sm text-gray-800 focus:ring-2 focus:ring-primary outline-none font-medium mb-3">
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="$set('maxPriceGhs', 100000)" class="rounded-full px-3.5 py-1.5 text-xs font-semibold bg-gray-900 text-white hover:bg-black transition-colors">Under 100K</button>
                        <button wire:click="$set('maxPriceGhs', 300000)" class="rounded-full px-3.5 py-1.5 text-xs font-semibold bg-gray-900 text-white hover:bg-black transition-colors">Under 300K</button>
                        <button wire:click="$set('maxPriceGhs', 600000)" class="rounded-full px-3.5 py-1.5 text-xs font-semibold bg-gray-900 text-white hover:bg-black transition-colors">Under 600K</button>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 py-4" x-data="{ mileage: {{ $maxMileage !== '' ? (int) $maxMileage : \App\Livewire\Cars\CarCatalogue::MAX_MILEAGE_CAP }} }">
                <button @click="open.mileage = !open.mileage" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Mileage
                ...
                <div x-show="open.mileage" class="mt-3">
                    <p class="text-sm font-semibold text-gray-700 mb-3" x-text="mileage >= {{ \App\Livewire\Cars\CarCatalogue::MAX_MILEAGE_CAP }} ? 'Any mileage' : Number(mileage).toLocaleString() + ' km or less'"></p>
                    <input
                        type="range"
                        min="0"
            ...
            {{-- Year --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.year = !open.year" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Year
                ...
                <div x-show="open.year" class="mt-3 flex items-center gap-3">
                    ...
                    <select wire:model.live="minYear" class="w-full bg-gray-100 border-none rounded-lg py-3 pl-3 pr-8 text-sm text-gray-800 text-center focus:ring-2 focus:ring-primary outline-none font-medium">
                    ...
                    <select wire:model.live="maxYear" class="w-full bg-gray-100 border-none rounded-lg py-3 pl-3 pr-8 text-sm text-gray-800 text-center focus:ring-2 focus:ring-primary outline-none font-medium">
            ...
            {{-- Fuel Type --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.fuel = !open.fuel" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Fuel Type
            ...
            {{-- Transmission --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.transmission = !open.transmission" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Transmission
            ...
            {{-- Country of Origin --}}
            <div class="border-t border-gray-100 py-4">
                <button @click="open.country = !open.country" class="w-full flex items-center justify-between text-sm font-bold text-gray-900">
                    Country of Origin
                ...
                <div x-show="open.country" class="mt-3">
                    <select wire:model.live="countryFilter" class="w-full bg-gray-100 border-none rounded-lg py-3 pl-3 pr-10 text-sm text-gray-800 focus:ring-2 focus:ring-primary outline-none font-medium">
    ...
    <div class="lg:col-span-3">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl font-bold text-gray-900">{{ $cars->total() }} {{ Str::plural('car', $cars->total()) }} found</h1>
            <div class="flex items-center gap-2">
                <button wire:click="$set('showMobileFilters', true)" class="lg:hidden flex items-center gap-1.5 bg-white border border-gray-300 rounded text-sm font-medium text-gray-700 py-2 px-3">
                    ...
                    Filters
                    @if ($hasActiveFilters)
                        <span class="inline-flex items-center justify-center w-4 h-4 rounded-full bg-primary text-white text-xs font-bold">&bull;</span>
                    @endif
                </button>
                <span class="text-sm text-gray-500 hidden sm:inline">Sort by</span>
                <select wire:model.live="sort" class="bg-gray-100 border-none rounded-lg py-2.5 pl-3 pr-10 text-sm font-medium text-gray-800 focus:ring-2 focus:ring-primary outline-none">
