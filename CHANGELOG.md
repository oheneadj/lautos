...
### Fixed
- Car and Order admin forms now reject a zero or negative price/shipping cost, and reject a car trim that doesn't belong to the car's selected model — previously an admin could submit a $0 car or pair a trim from one model with a different model (e.g. a Corolla with a Camry's "SE" trim) since the client-side model/trim filtering had no matching server-side validation.
