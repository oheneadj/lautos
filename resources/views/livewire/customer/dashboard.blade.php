    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                <h1 class="text-[28px] font-semibold text-base-content leading-tight flex flex-wrap items-center gap-3">
                    <span>
                    ...
                    @if ($this->user->kyc_status === \App\Enums\KycStatus::Verified)
                        <span class="inline-flex items-center gap-1 rounded-full bg-success/10 px-2.5 py-1 text-[11px] font-bold text-success border border-success/20 tracking-wide uppercase translate-y-0.5">
                    ...
                    @else
                        <span class="inline-flex items-center gap-1 rounded-full bg-warning/10 px-2.5 py-1 text-[11px] font-bold text-warning-content dark:text-warning border border-warning/20 tracking-wide uppercase translate-y-0.5">
                            <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3Z" /></svg>
                            Unverified
                        </span>
                    @endif
                </h1>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <x-ui.card plain class="bg-error text-white border border-error/20 p-4 flex items-center gap-4">
            ...
            {{-- Saved Cars --}}
            <x-ui.card class="p-4 flex items-center gap-4">
        ...
        <div class="grid gap-4 lg:grid-cols-3">
            {{-- Order Pipeline --}}
            <x-ui.card subtitle="{{ __('ALL TIME') }}" title="{{ __('Order Pipeline') }}">
                <x-slot:icon>
                        ...
                        <div class="relative ml-2 space-y-3 mt-2">
                            ...
                            @foreach ($this->ordersByStage as $stage)
                                ...
                                <div class="relative flex items-center gap-4 group">
                                    ...
                                    <!-- Stage Card -->
                                    <a href="{{ route('dashboard.orders', ['statusFilter' => $stage['value']]) }}" wire:navigate class="flex-1 flex items-center justify-between border border-base-content/5 rounded-xl px-4 py-2.5 hover:bg-base-200/50 hover:border-primary/20 transition-all duration-200 cursor-pointer">
            ...
            <x-ui.card title="{{ __('Recent Orders') }}" actionLabel="{{ __('View all →') }}" :actionHref="route('dashboard.orders')" class="lg:col-span-2">
        ...
        {{-- Bottom Row: Watchlist, Broker, Blog --}}
        <div class="grid gap-4 lg:grid-cols-3">
            {{-- Watchlist Preview --}}
            <x-ui.card title="{{ __('Recent Saved Cars') }}" actionLabel="{{ __('View all →') }}" :actionHref="route('dashboard.saved-cars')" headerBorder class="flex flex-col h-full">
            ...
            {{-- Need Help? — driven by real settings + the actual support ticket feature, not a fake agent. --}}
            <x-ui.card class="flex flex-col relative overflow-hidden h-full">
                ...
                <div class="px-5 pt-8 pb-5 flex-1 flex flex-col items-center text-center relative z-10 mt-2">
                    <div class="w-16 h-16 rounded-full bg-white border-4 border-base-100 shadow-sm flex items-center justify-center text-primary mb-3">
                        <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    </div>
                    <h3 class="text-[16px] font-bold text-base-content">{{ __('Need Help?') }}</h3>
                    <p class="text-[12px] font-medium text-base-content/50 mb-4">{{ __('Our team typically replies within 24 hours') }}</p>

                    <p class="text-[13px] text-base-content/70 mb-5 leading-relaxed">
                        {{ __('Open a support ticket and our team will help with your order, shipping, or anything else.') }}
                    </p>

                    <a href="{{ route('dashboard.support') }}" wire:navigate
                        class="w-full mb-2 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors text-[13px] font-bold">
                        {{ $this->openTicketsCount > 0 ? __('View Support Tickets (:count open)', ['count' => $this->openTicketsCount]) : __('Open a Support Ticket') }}
                    </a>

                    @php
                        $supportEmail = \App\Models\Setting::get('contact_email');
                        $supportWhatsapp = preg_replace('/\D/', '', (string) \App\Models\Setting::get('whatsapp_number', ''));
                    @endphp
                    <div class="mt-auto w-full grid grid-cols-2 gap-2">
                        @if ($supportEmail)
                            <a href="mailto:{{ $supportEmail }}" class="flex items-center justify-center gap-2 py-2 rounded-lg bg-base-200 hover:bg-base-300 transition-colors text-[12px] font-bold text-base-content">
                                <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" /></svg>
                                Email
                            </a>
                        @endif
                        @if ($supportWhatsapp)
                            <a href="https://wa.me/{{ $supportWhatsapp }}" target="_blank" class="flex items-center justify-center gap-2 py-2 rounded-lg bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 transition-colors text-[12px] font-bold">
            ...
            <x-ui.card class="flex flex-col h-full">
                <div class="p-4 flex-1 flex flex-col gap-4">
                    @forelse ($this->latestBlogPosts as $post)
                        <a href="{{ route('blog.show', $post->slug) }}" wire:navigate class="flex flex-col group">
                            ...
                            <div class="pt-3">
                                <h3 class="text-[16px] font-bold text-base-content leading-snug group-hover:text-primary transition-colors duration-200 line-clamp-2">{{ $post->title }}</h3>
                                <p class="text-[11px] font-medium text-base-content/40 mt-1.5">{{ $post->published_at->format('M d, Y') }}</p>
                                <a href="{{ route('blog.show', $post->slug) }}" wire:navigate class="mt-2 text-[12px] font-bold text-primary hover:underline">{{ __('Read More') }}</a>
                            </div>
                        </a>
                    @empty
