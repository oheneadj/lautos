    ...
    <div class="max-w-[85rem] mx-auto px-4 lg:px-8 mb-8">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6 pb-4 border-b border-gray-200">
            ...
            <div class="flex items-center gap-2 flex-wrap w-full md:w-auto">
                <button wire:click="$set('categoryFilter', '')" class="text-[13px] font-bold px-4 py-2 rounded-full transition-colors {{ empty($categoryFilter) ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">All</button>
                @foreach ($categories as $category)
                    <button wire:click="$set('categoryFilter', '{{ $category->slug }}')" class="text-[13px] font-bold px-4 py-2 rounded-full transition-colors {{ $categoryFilter === $category->slug ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
            ...
            <div class="relative w-full md:w-72 shrink-0">
                ...
                <input
                    ...
                    class="w-full pl-9 pr-4 py-2.5 text-[13px] bg-gray-50 border border-gray-200 rounded-lg outline-none font-medium placeholder:text-gray-400 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
            </div>
        </div>
    </div>
        ...
        <section class="max-w-[85rem] mx-auto px-4 lg:px-8">
            @if ($posts->isEmpty())
                <div class="text-center py-20">
                    <p class="text-[15px] font-semibold text-gray-500">No articles found</p>
                    <p class="text-[13px] text-gray-400 mt-1">Try adjusting your search or category filter.</p>
                </div>
            @else
                    ...
                    @foreach ($posts as $post)
                        <a href="{{ route('blog.show', $post->slug) }}" class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 flex flex-col h-full flex flex-col overflow-hidden">
                            ...
                            <div class="p-6 flex flex-col flex-1">
                                @if ($post->category)
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary border border-primary/20 w-fit mb-3">{{ $post->category->name }}</span>
                                @endif
                                <h3 class="text-[16px] font-bold text-gray-900 leading-snug group-hover:text-primary transition-colors">{{ $post->title }}</h3>
                                @if ($post->excerpt)
                                    <p class="text-[14px] text-gray-500 mt-2 leading-relaxed line-clamp-3">{{ $post->excerpt }}</p>
                                @endif
                                <div class="flex items-center gap-2 mt-auto pt-4 border-t border-gray-100">
                                    <span class="text-[12px] text-gray-400 font-medium">{{ $post->published_at->format('M j, Y') }}</span>
                                    @if ($post->author)
                                        <span class="text-gray-300 text-[12px]">&bull;</span>
                                        <span class="text-[12px] text-gray-400 font-medium">{{ $post->author->name }}</span>
                                    @endif
                                    <span class="text-gray-300 text-[12px]">&bull;</span>
                                    <span class="text-[12px] text-gray-400 font-medium">{{ $post->read_time }} min read</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
        ...
        <section class="max-w-[85rem] mx-auto px-4 lg:px-8 mb-16">
            ...
            <a href="{{ route('blog.show', $featuredNews->slug) }}" class="relative block w-full h-[400px] md:h-[500px] rounded-2xl overflow-hidden mb-6 group cursor-pointer">
                ...
                <div class="absolute bottom-0 left-0 p-8 md:p-12 w-full max-w-4xl">
                    ...
                    <p class="text-white/80 text-[15px] md:text-[16px] font-medium mb-6 line-clamp-2 md:line-clamp-none max-w-2xl">
                        {{ $featuredNews->excerpt }}
                    </p>
                    <div class="flex items-center gap-4">
                        <span class="bg-white text-gray-900 font-bold px-6 py-3 rounded-full text-[14px] hover:bg-gray-100 transition-colors inline-block">
                            Read more
                        </span>
                        <span class="text-white/70 text-[13px] font-medium">{{ $featuredNews->read_time }} min read</span>
                    </div>
                </div>
            </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <a href="{{ route('blog.show', $sub->slug) }}" class="flex gap-4 group">
                    ...
                    <div>
                        @if($sub->category)
                            <span class="text-primary text-[10px] font-black uppercase tracking-widest mb-1 block">{{ $sub->category->name }}</span>
                        @endif
                        <h4 class="font-bold text-gray-900 text-[16px] leading-snug group-hover:text-primary transition-colors mb-2">{{ $sub->title }}</h4>
                        <p class="text-[13px] text-gray-500 line-clamp-3">{{ $sub->excerpt }}</p>
                    </div>
                </a>
                @endforeach
            </div>
                ...
                <div class="lg:col-span-8">
                    ...
                    <a href="{{ route('blog.show', $topStory->slug) }}" class="mb-10 group block cursor-pointer">
                        ...
                        @if($topStory->category)
                            <span class="text-primary text-[12px] font-black uppercase tracking-widest mb-2 block">{{ $topStory->category->name }}</span>
                        @endif
                        ...
                        <div class="text-[12px] text-gray-400 font-medium flex items-center gap-2">
                            @if($topStory->author)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10 mb-8">
                        ...
                        <a href="{{ route('blog.show', $latest->slug) }}" class="group block cursor-pointer">
                            ...
                            @if($latest->category)
                                <span class="text-primary text-[11px] font-black uppercase tracking-widest mb-2 block">{{ $latest->category->name }}</span>
                            @endif
                    ...
                    <div class="mb-12">
                        ...
                        <div class="space-y-6">
                            @foreach($featuredStories as $index => $story)
                                @if($index === 0)
                                    ...
                                    <a href="{{ route('blog.show', $story->slug) }}" class="group block">
                                        ...
                                        @endif
                                        <h4 class="text-[15px] font-bold text-gray-900 leading-snug group-hover:text-primary transition-colors">{{ $story->title }}</h4>
                                        <div class="text-[11px] text-gray-400 font-medium mt-1">{{ $story->published_at->format('M d, Y') }}</div>
                                    </a>
                                @else
                                    ...
                                    <a href="{{ route('blog.show', $story->slug) }}" class="flex gap-3 group">
                                        ...
                                        <div>
                                            @if($story->category)
                                                <span class="text-primary text-[9px] font-black uppercase tracking-widest mb-1 block">{{ $story->category->name }}</span>
                                            @endif
                                            <h4 class="text-[13px] font-bold text-gray-900 leading-tight group-hover:text-primary transition-colors">{{ $story->title }}</h4>
                                            <div class="text-[10px] text-gray-400 font-medium mt-1">{{ $story->published_at->format('M d, Y') }}</div>
                                        </div>
                                    </a>
                                @endif
                            @endforeach
                            ...
                            <div class="bg-[#1a1c23] rounded-xl p-6 text-center mt-6 shadow-xl shadow-gray-200/50 border border-gray-800">
                                <div class="text-[11px] text-primary font-black uppercase tracking-widest mb-2">Custom Imports</div>
                                <h4 class="text-white font-bold text-lg leading-tight mb-4">Can't find what you're looking for? Order it.</h4>
                                <a href="{{ route('contact') }}" class="w-full inline-block bg-primary text-white font-bold py-3 rounded-lg text-[13px] hover:bg-red-700 transition-colors">Start Custom Order</a>
                            </div>
