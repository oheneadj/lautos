                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-gray-100 pb-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <button id="copy-link-btn" class="relative w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center hover:bg-gray-200 transition-colors" title="Copy link">
                            ...
                            <div id="copy-tooltip" style="display: none;" class="absolute -top-8 bg-gray-900 text-white text-xs font-bold px-2 py-1 rounded whitespace-nowrap">
                                Copied!
                            </div>
                        </button>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
                ...
                <div class="lg:col-span-8">
                    ...
                    <div class="mb-8">
                        ...
                        <p class="text-xs text-gray-400 font-medium mt-3 text-right">Livingston Autos / Featured Image</p>
                    </div>
                    @endif
                    ...
                    <div class="border-t border-gray-100 pt-10 mb-16">
                        ...
                        {{-- Mock Video Embed --}}
                        <div class="mb-10">
                            <h3 class="text-sm font-bold text-gray-900 mb-4">Related Video:</h3>
                            <div class="relative w-full aspect-video rounded-xl overflow-hidden group cursor-pointer shadow-lg shadow-gray-200/50 border border-gray-100">
                                ...
                                <div class="absolute top-0 left-0 w-full p-4 bg-gradient-to-b from-black/60 to-transparent flex items-center justify-between">
                                    <div class="text-white font-bold text-base truncate">Everything you need to know about {{ $post->category ? $post->category->name : 'importing' }}</div>
                                    <div class="text-white/80 text-xs flex items-center gap-1 shrink-0"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg> Share</div>
                                </div>
                                ...
                                <div class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1 text-gray-900 font-bold text-xs flex items-center gap-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($featuredStories as $story)
                                <a href="{{ route('blog.show', $story->slug) }}" class="group block">
                                    ...
                                    @endif
                                    <h4 class="text-base font-bold text-gray-900 leading-snug group-hover:text-primary transition-colors mb-2">{{ $story->title }}</h4>
                ...
                <div class="lg:col-span-4">
                    ...
                    <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100 mb-10 shadow-sm">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <div class="text-xs text-gray-400 font-bold uppercase tracking-widest mb-1">Import Experts</div>
                        ...
                        <a href="{{ route('contact') }}" class="block w-full bg-gray-900 text-white text-center font-bold py-3 rounded-lg text-sm hover:bg-gray-800 transition-colors">
                            Contact Sales Team
                        </a>
                    </div>
                    ...
                    <div class="mb-10">
                            ...
                            @foreach($latestNews as $news)
                                <a href="{{ route('blog.show', $news->slug) }}" class="group block cursor-pointer flex gap-4">
                                    <div class="flex flex-col justify-center flex-1">
                                        @if($news->category)
                                            <span class="text-primary text-xs font-black uppercase tracking-widest mb-1 block">{{ $news->category->name }}</span>
                                        @endif
                                        <h4 class="text-sm font-bold text-gray-900 leading-snug group-hover:text-primary transition-colors mb-1">{{ $news->title }}</h4>
                                        <div class="text-xs text-gray-400 font-medium">By {{ $news->author->name ?? 'Livingston' }} &bull; {{ $news->published_at->format('M j, Y') }}</div>
                                    </div>
                        ...
                        <div class="flex justify-end mt-4">
                            <a href="{{ route('blog.index') }}" class="text-sm font-bold text-primary hover:underline">See all latest news &rarr;</a>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="mb-10">
                        <h2 class="text-xl font-bold mb-5">Browse by category</h2>
                        <div class="space-y-3">
                            @foreach($categories as $category)
                                {{-- Same bg-white/border/rounded-lg/hover:shadow-lg card treatment used for car-card and other cards across the site. --}}
                                <a href="{{ route('blog.index', ['category' => $category->slug]) }}"
                                    class="flex items-center justify-between px-4 py-3.5 bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-shadow duration-300 group">
                                    <span class="text-sm font-bold text-gray-900 group-hover:text-primary transition-colors">{{ $category->name }}</span>
                                    <span class="text-xs font-bold text-gray-600 bg-gray-100 rounded-full px-2.5 py-0.5">{{ $category->posts_count }}</span>
                                </a>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="sticky top-24">
                        <div class="w-full aspect-square bg-[#da251d] rounded-2xl overflow-hidden relative shadow-xl shadow-red-200/50 group cursor-pointer">
                            ...
                            <div class="absolute inset-0 p-8 flex flex-col items-center justify-center text-center">
                                ...
                                <p class="text-white/90 text-sm font-bold uppercase tracking-widest mb-6">On Port Clearance Fees</p>

                                <span class="bg-gray-900 text-white text-sm font-bold py-3 px-8 rounded-full hover:bg-black transition-colors">Claim Offer</span>
                            </div>
                        </div>
                    </div>
