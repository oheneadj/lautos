    ...
    <div class="min-h-screen flex items-center justify-center p-6 md:p-10">
        <div class="w-full max-w-[460px] flex flex-col gap-8">
            ...
            <div class="flex flex-col items-center gap-3">
                @php $authLogoPath = \App\Models\Setting::get('site_logo_path'); @endphp
                <a href="{{ route('home') }}" wire:navigate class="hover:opacity-80 transition-opacity duration-200">
                    <div class="flex items-center gap-2">
                        @if ($authLogoPath)
                            <img src="{{ \Illuminate\Support\Facades\Storage::url($authLogoPath) }}" alt="{{ config('app.name') }}" class="h-7 w-auto">
                        @else
                        ...
                        @endif
                        <span class="text-[15px] font-semibold text-base-content">{{ config('app.name') }}</span>
                    </div>
                </a>
